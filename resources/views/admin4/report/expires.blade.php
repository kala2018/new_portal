...
<article class="page-container">
    <form action="" method="post" class="form form-horizontal" id="form-member-add">
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3">权限时效（单位小时）：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="" placeholder="" id="expire" name="expire">
            </div>
        </div>
        <div class="row cl">
            ...
            submitHandler:function(form){
                $(form).ajaxSubmit({
                    ...
                    success: function(data){
                        console.log(data);
                        if(data == '1'){
                            layer.msg('过期时间修改修改成功!',{icon:1,time:1000},function(){
                                var index = parent.layer.getFrameIndex(window.name);
                                parent.window.location = parent.window.location;
                                parent.layer.close(index);
                            });
                        }else{
                            layer.msg('过期时间修改修改失败',{icon:2,time:2000});
                        }
                    },
